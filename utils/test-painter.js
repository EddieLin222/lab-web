...
function getUV(point, vA, vB, vC, uvA, uvB, uvC) {
    // 计算三角形的面积
    const area = new THREE.Triangle(vA, vB, vC).getArea();

    // 计算重心坐标
    const areaABP = new THREE.Triangle(vA, vB, point).getArea();
    const areaBCP = new THREE.Triangle(vB, vC, point).getArea();
    const areaCAP = new THREE.Triangle(vC, vA, point).getArea();

    const u = (areaBCP / area) * uvA.x + (areaCAP / area) * uvB.x + (areaABP / area) * uvC.x;
    const v = (areaBCP / area) * uvA.y + (areaCAP / area) * uvB.y + (areaABP / area) * uvC.y;
